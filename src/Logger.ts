...
export default class Logger {
	...
	public static Error(msg: string, { file, line }: CallerLocation) {
		if (
			![Verbosity.DEBUG, Verbosity.ERRORS_ONLY, Verbosity.ERRORS_WARNINGS, Verbosity.EVERYTHING].includes(
				this._verbosity
			)
		)
			return;
	...
	public static Warning(msg: string, { file, line }: { file: string; line: number }) {
		if (![Verbosity.DEBUG, Verbosity.ERRORS_WARNINGS, Verbosity.EVERYTHING].includes(this._verbosity)) return;
	...
	public static Info(msg: string, { file, line }: { file: string; line: number }) {
		if (![Verbosity.DEBUG, Verbosity.SILENT, Verbosity.EVERYTHING].includes(this._verbosity)) return;
	...
	public static Debug(msg: string, { file, line }: { file: string; line: number }) {
		if (![Verbosity.DEBUG].includes(this._verbosity)) return;
